    ...
    <section id="noticias" className="py-24 bg-background">
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex items-end justify-between mb-12">
          ...
          <Link href={ROUTES.noticiasPagina} className="hidden sm:flex items-center gap-2 text-sm text-accent hover:text-white transition-colors">
            Ver todas <ChevronRight size={16} />
          </Link>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Link href={ROUTES.noticiasPagina} className="lg:col-span-2 group cursor-pointer">
          ...
          <div className="flex flex-col gap-4">
            {noticias.slice(1).map((n) => (
              <Link href={ROUTES.noticiasPagina} key={n.id} className="group flex gap-4 p-4 rounded-lg bg-card border border-white/5 hover:border-accent/30 transition-all duration-200 cursor-pointer">
