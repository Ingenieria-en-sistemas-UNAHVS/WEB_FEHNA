...
export function PatrocinadoresSection({ patrocinadores, visible }: Props) {
  ...
  //Revisa si la cantidad de patrocinadores es mayor a 12, sino, los empieza a duplicar para llenar el carrucel
  const minimoElementos = 12;
  const patrocinadoresRepetidos: PatrocinadorRow[] = [];

  while (patrocinadoresRepetidos.length < minimoElementos) {
    patrocinadoresRepetidos.push(...patrocinadores);
  }
      ...
      {patrocinadores.length === 0 ? (
        ...
        <div className="group relative w-full overflow-hidden">
          ...
          <div className="patrocinadores-track flex w-max items-center gap-20 group-hover:[animation-play-state:paused]">
            {patrocinadoresRepetidos.map((p, i) => (
