    ...
    <section id="calendario" className="py-24 bg-secondary">
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-12">
          ...
          <Link href={ROUTES.calendarioPagina} className="self-start sm:self-auto flex items-center gap-2 px-5 py-3 bg-accent text-[#061529] font-bold rounded text-sm hover:bg-white transition-all duration-200" style={{ fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: "0.05em" }}>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {filtered.map((ev) => (
              <Link href={ROUTES.calendarioPagina} key={ev.id} className="flex items-start gap-4 p-4 rounded-lg bg-card border border-white/5 hover:border-accent/30 transition-all duration-200 group cursor-pointer">
